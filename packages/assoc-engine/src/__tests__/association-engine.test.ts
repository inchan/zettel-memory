...
class StubSearchEngine implements SearchEngineAdapter {
  ...
  constructor(
    private readonly _result: EnhancedSearchResult,
    private readonly _backlinks: Record<string, LinkRelation[]>,
    private readonly _graph: Record<string, LinkGraphNode[]>
  ) {}
...
describe('AssociationEngine', () => {
  ...
  it('enforces timeout limits', async () => {
    const stub = new StubSearchEngine(baseResult, backlinks, graph);
    const engine = new AssociationEngine(stub, {
      ...engineConfig,
      timeoutMs: 1,
    });

    const slowSearch = jest
      .spyOn(stub, 'search')
      .mockImplementationOnce(
        async (_query: string, _options?: SearchOptions) => {
  ...
  it('validates request payloads with zod schema', async () => {
    ...
    await expect(
      engine.generateRecommendations(invalidPayload as any)
